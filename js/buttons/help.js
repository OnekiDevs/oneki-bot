    ...
    const server = client.servers.get(interact.guildId);
        fetch(`https://oneki.herokuapp.com/api/${server.lang}/cmd/categories`).then((r) => r.json()).then((categories) => {
            fetch(`https://oneki.herokuapp.com/api/${server.lang}/cmd/${interact.values[0]}`).then((r) => r.json()).then(async category=>{
                const embed = new MessageEmbed().setTitle(`${await client.util.replace(lang.embed.title, [{match:"{bot}", replace:interact.guild.me.displayName}])}`).setDescription(`${await client.util.replace(lang.embed.description, [{match:"{type}", replace:interact.values[0]}])}`), menu = new MessageSelectMenu().setCustomId('help').setPlaceholder(lang.menu.placeholder), lang = client.util.lang({lang:server.lang, route:'commands/help'})
