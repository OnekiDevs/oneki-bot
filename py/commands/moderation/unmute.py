...
async def unmute(ctx, member : tools.discord.Member):
    ...
    try:
        ...
        await tools.give_list_roles(ctx.guild, member, dic[f"{tools.get_user(member).id}"]["roles"])
        collection_times.delete(f"{ctx.guild.id}", f"mute.{tools.get_user(member).id}")
