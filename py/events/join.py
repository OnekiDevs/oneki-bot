...
async def on_member_join(message):
    ...
    channel = tools.bot.get_channel(int(collection.get("bienvenidas", "channel")))

    img = tools.Image.open("src/welcome.png")
    font = tools.ImageFont.truetype("Tools/arial.ttf", 30)
    tools.ImageDraw.ImageDraw(img).text((210, 155), text = collection.get("bienvenidas", "mensaje").format(message.name, message.guild.name), font = font, fill = "white")
